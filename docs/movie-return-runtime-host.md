...
The enabled-mod list must contain only `NeonRewindMovieReturnProbe`.
The NeonRewind mod source, controlling `mods.txt`, diagnostic output, and run metadata remain under an ignored repository `.local` directory.
