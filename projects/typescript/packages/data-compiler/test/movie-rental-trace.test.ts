...
test("rejects a changed typed rental new-day entrypoint", () => {
  ...
  const entry = findNode(trace, "Weather - New Day Event", 18);
...
test("rejects a changed typed readiness transfer", () => {
  const trace = createRentalFunctionTrace();
  const append = findNode(trace, "ExecuteUbergraph_RentSystem", 1854);
  const destination = findChild(trace, "ExecuteUbergraph_RentSystem", append.nodeIndex, "Parameters[0]");
  destination.symbol = "Cartridge Base out for Rent";
...
test("rejects a changed typed first-attempt override", () => {
  const trace = createRentalFunctionTrace();
  const select = findNode(trace, "Get Random List Of Cartridges From Rent List", 290);
  const probability = findChild(
    trace,
    "Get Random List Of Cartridges From Rent List",
    select.nodeIndex,
    "Parameters[0]",
  );
...
test("rejects typed selection that no longer adds unique candidates", () => {
  const trace = createRentalFunctionTrace();
  const addUnique = findNode(trace, "Get Random List Of Cartridges From Rent List", 782);
...
test("rejects a changed typed weighted-failure route", () => {
  const trace = createRentalFunctionTrace();
  const branch = findNode(trace, "Get Random List Of Cartridges From Rent List", 562);
...
test("rejects a changed typed empty-result assignment", () => {
  const trace = createRentalFunctionTrace();
  const assignment = findNode(trace, "Get Random List Of Cartridges From Rent List", 976);
