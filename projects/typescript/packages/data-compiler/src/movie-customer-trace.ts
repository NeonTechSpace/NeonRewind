import type { BlueprintTraceEvidence } from "@neonrewind/core";
...
export const customerCallerClassName = "AI_Client_Character_C";
export const customerCallerFunctionName =
  "Initial creation - Get if I have Product to return";
...
const eventGraphFunctionName = "ExecuteUbergraph_AI_Client_Character";
const selectionFunctionName = "Get Random List Of Cartridges From Rent List";
const selectorStatementIndexes = [465, 519] as const;
const consoleFoundSymbol =
  "CallFunc_Get_Random_Console_From_Rent_Console_List_Find_a_product";
const selectorFoundSymbol =
  "CallFunc_Get_Random_List_Of_Cartridges_From_Rent_List_Find_a_product";
const selectorItemsSymbol =
  "CallFunc_Get_Random_List_Of_Cartridges_From_Rent_List_Item_founded";
const loopCounterSymbol = "Temp_int_Loop_Counter_Variable";
const loopConditionSymbol = "CallFunc_Less_IntInt_ReturnValue";
...
export function assertMovieCustomerTrace(
  ...
  assertCall(eventGraph, 35, "Generate Random movie favorite", "local-virtual", 0);
  assertCall(
    eventGraph,
    49,
...
function assertCustomerControlFlow(
  ...
  assertSymbolChild(lessThan, customer, "Parameters[1]", "CallFunc_Array_Length_ReturnValue");
  ...
  const readyRemoval = findCall(
    customer,
    987,
    "Remove Product From Rent Out Ready List",
    "local-virtual",
    2,
  );
...
function assertTraceStructure(trace: BlueprintFunctionTraceArtifact): void {
  const expectedNames = [
    "Attach Basket to Client",
    "Attach Child to Skeletal",
    eventGraphFunctionName,
    "Generate Random movie favorite",
    customerCallerFunctionName,
    "ReceiveBeginPlay",
    "Update Physics Handler for Basket",
  ];
  if (
  ...
  const entrypointCount = nodes.filter(
    (node) =>
      node.call?.functionName.startsWith("ExecuteUbergraph_") === true &&
