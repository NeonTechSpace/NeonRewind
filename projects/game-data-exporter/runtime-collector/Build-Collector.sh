...
cmake \
    ...
    -DNEONRETROREWIND_UE4SS_SOURCE="$ue4ss_source" || fail "CMake configuration failed."

cmake \
    --build "$build_directory" \
    --target NeonRetroRewindMovieReturnCollector \
    --parallel "$parallel_jobs" || fail "Collector build failed."

artifact="$build_directory/artifact/NeonRetroRewindMovieReturnCollector/dlls/main.dll"
