...
local RENT_CLASS = "/Game/ExampleProject/core/blueprint/ExampleQueueSystem/ExampleQueueSystem.ExampleQueueSystem_C"
local CUSTOMER_CLASS = "/Game/ExampleProject/core/ai/pawn/ExampleActor.ExampleActor_C"
...
local function refresh_rental_manager(candidate)
    ...
    elseif not valid_object(rental_manager) then
        local ok, found = pcall(FindFirstOf, "ExampleQueueSystem_C")
    ...
    if rental.found then
        rental.queues = json_object({
            rented = snapshot_array(rental_manager, "Example Active Items"),
            readyToReturn = snapshot_array(rental_manager, "Example Ready Items"),
        })
    end
end
...
define_hook(
    "movie-readiness",
    RENT_CLASS .. ":Prepare Example Items",
    "blueprint-post-only")
define_hook(
    "movie-selector",
    RENT_CLASS .. ":Select Example Items",
    "blueprint-post-only")
define_hook(
    "customer-return",
    CUSTOMER_CLASS .. ":Initialize Example Return",
    "blueprint-post-only")
refresh_rental_manager(nil)

local found_customers_ok, found_customers = pcall(FindAllOf, "ExampleActor_C")
